...
from indianrailwaysapp.yatrigan.api import errors as IrError
...
class ShopListApi(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if shops.filter(is_active=True, is_verified=True).count() != 0:
            ...
            response_data.update(IrError.irShopListInActiveNotVerified(shops[0].station.name, shops[0].station.code))
